...
{
    my ($self) = @_;
    my $submod = $self->get_service_name() || 'Ontology';
    my $loggers = {};
...
sub call_method {
    ...
    my $ctx = Bio::KBase::OntologyService::ServiceContext->new($self->{loggers}->{userlog},
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service Ontology.");
    }
